...
public class SpringCloudS3Service {
    ...
    public List<S3ObjectSummary> listObjects(String bucketName) {
		...
		try {
			objects = listS3Objects(bucketName);
		} catch (IOException e) {
    ...
    public void uploadObject(String bucketName, File file) {
        String s3Url = "s3://" + bucketName + "/" + file.getName();
        try {
    ...
    public void deleteBucket(String bucketName) {
 	...
 	private List<S3ObjectSummary> listS3Objects(String bucketName) throws IOException {
        ListObjectsV2Result result = amazonS3.listObjectsV2(bucketName);
        return (result != null ? result.getObjectSummaries() : null);
 	}
}
